...
var theTests = []struct {
	...
	{"show res cal with params", "/admin/reservations-calendar?y=2026&m=1", "GET", http.StatusOK},
}
...
var postReservationTests = []struct {
...
}{
	{
		...
		reservation: models.Reservation{
			RoomID:    1,
			StartDate: time.Date(2050, time.January, 1, 0, 0, 0, 0, time.Now().Location()),
			EndDate:   time.Date(2050, time.January, 2, 0, 0, 0, 0, time.Now().Location()),
		},
	},
	{
...
func TestPostReservation(t *testing.T) {
	for _, e := range postReservationTests {
		...
		ctx := getCtx(req)
		req = req.WithContext(ctx)
		empty_res := models.Reservation{}
		if e.reservation != empty_res {
			session.Put(ctx, "reservation", e.reservation)
		}
...
var testAvailabilityJSONData = []struct {
...
}{
	{
		...
		expectedOK:      false,
		expectedMessage: "Invalid start date",
	}, {
		...
		expectedOK:      false,
		expectedMessage: "Invalid end date",
	}, {
...
var bookRoomTests = []struct {
	...
	{
		name:               "invalid start date",
		url:                "/book-room?s=invalid1&e=2050-01-02&id=1",
		expectedStatusCode: http.StatusSeeOther,
	},
	{
		name:               "invalid end date",
		url:                "/book-room?s=2050-01-01&e=invalid&id=1",
		expectedStatusCode: http.StatusSeeOther,
	},
	{
		name:               "invalid room id",
